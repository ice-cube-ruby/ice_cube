module IceCube
  module Validations::MinutelyBySetPos
    ...
    class Validation
      ...
      def type
        # Use the smallest expanded unit so we don't skip intra-minute candidates.
      ...
      def validate(step_time, start_time)
        # Define the current minute window so BYSETPOS is applied per minute.
        start_of_minute = TimeUtil.build_in_zone(
          [step_time.year, step_time.month, step_time.day, step_time.hour, step_time.min, 0], step_time
        )
        end_of_minute = TimeUtil.build_in_zone(
          [step_time.year, step_time.month, step_time.day, step_time.hour, step_time.min, 59], step_time
        )
        ...
        occurrences = new_schedule.occurrences_between(start_of_minute, end_of_minute)
