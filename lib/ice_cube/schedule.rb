...
module IceCube
  ...
  class Schedule
    ...
    def enumerate_occurrences(opening_time, closing_time = nil, options = {}, &block)
      ...
      spans_offset = spans ? duration : 0
      Enumerator.new do |yielder|
        reset
        t1 = full_required? ? start_time : opening_time
        loop do
          break unless (t0 = next_time(t1, closing_time, spans_offset))
          break if closing_time && t0 > closing_time
          if (spans ? (t0.end_time > opening_time) : (t0 >= opening_time))
            yielder << (block_given? ? block.call(t0) : t0)
          end
          break unless (t1 = next_time(t0 + 1, closing_time, spans_offset))
    ...
    def next_time(time, closing_time, spans_offset)
      loop do
        min_time = recurrence_rules_with_implicit_start_occurrence.reduce(nil) do |min_time, rule|
          begin
            new_time = rule.next_time(time, start_time, min_time || closing_time, spans_offset)
