...
describe Schedule, 'to_yaml' do
  ...
  it 'should respond to .to_yaml' do
    ...
    schedule.addRecurrenceRule Rule.daily.until(Date.today)
    #check assumption
  ...
  it 'should respond to a single date event' do
    schedule = Schedule.new(Date.today)
    schedule.addRecurrenceDate(Date.today + 2)
    #check assumptions
  ...
  it 'should not return anything when given a single date and the same exclusion date' do
    schedule = Schedule.new(Date.today)
    schedule.addRecurrenceDate(Date.today + 2)
    schedule.addExceptionDate(Date.today + 2)
    #check assumption
  ...
  it 'should return properly with a combination of a recurrence and exception rule' do
    schedule = Schedule.new(Date.today)
    schedule.addRecurrenceRule Rule.daily # every day
    schedule.addExceptionRule Rule.weekly.day_of_week(:monday, :tuesday, :wednesday) # except these
  ...
  it 'should be able to exclude a certain date from a range' do
    ...
    schedule.addExceptionDate(Date.today + 1) # all days except tomorrow
    # check assumption
  ...
  it 'should be able to handle a complex rule from the README' do
    ...
    schedule.addRecurrenceRule Rule.weekly.day_of_week(:sunday)
    #check assumption
