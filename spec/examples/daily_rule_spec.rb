...
describe IceCube::DailyRule, 'occurs_on?' do
  ...
  context system_time_zone: 'America/Vancouver' do
    ...
    it 'should include nearest time in DST start hour' do
      schedule = IceCube::Schedule.new(t0 = Time.local(2013, 3, 9, 2, 30, 0))
      schedule.add_recurrence_rule IceCube::Rule.daily
    ...
    it 'should not skip times in DST end hour' do
      schedule = IceCube::Schedule.new(t0 = Time.local(2013, 11, 2, 2, 30, 0))
      schedule.add_recurrence_rule IceCube::Rule.daily
      schedule.first(3).should == [
  ...
  it 'should produce the correct days for @interval = 1' do
    ...
    schedule.add_recurrence_rule IceCube::Rule.daily
    #check assumption
    times = schedule.occurrences(t0 + 2 * IceCube::ONE_DAY)
    times.size.should == 3
    times.should == [t0, t0 + IceCube::ONE_DAY, t0 + 2 * IceCube::ONE_DAY]
  end
  ...
  it 'should produce the correct days for @interval = 2' do
    ...
    times = schedule.occurrences(t0 + 5 * IceCube::ONE_DAY)
    times.size.should == 3
    times.should == [t0, t0 + 2 * IceCube::ONE_DAY, t0 + 4 * IceCube::ONE_DAY]
  end

  it 'should produce the correct days for @interval = 2 when crossing into a new year' do
    schedule = IceCube::Schedule.new(t0 = Time.utc(2011, 12, 29))
    schedule.add_recurrence_rule IceCube::Rule.daily(2)
    #check assumption (3) -- (1) 2 (3) 4 (5) 6
    times = schedule.occurrences(t0 + 5 * IceCube::ONE_DAY)
    times.size.should == 3
